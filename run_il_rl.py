...
default_cfg_file = 'configs/train/default.ini'
cfg_file = sys.argv[1]
...
if __name__ == '__main__':
    ...
    for local_job_id, params in product_dict(**args):
        ...
        env_name = ("{id}_"
                    ...
                    "--sz-{sampling_zone}"
                    "--nb-ep-{nb_epochs}"
                    "--s-enc-{state_encoding}"
                    "--hsz-{hidden_size}"
                    "--rl-mode-{reinforce_mode}"
                    "--rl-norm-{reinforce_norm}"
                    "--b-im-ep-{beta_im_ep}-s-{beta_im_start}-e-{beta_im_end}"
                    "--b-rl-{beta_rl}"
                    "--me-{max_episodes}").format(**params)

        env_name = env_name.replace('/', '-')
        env_name = env_name.replace(' ', '-')

        # Format the exclude rooms
        #_temp = params['start_state_exclude_rooms'].split()
        #params['start_state_exclude_rooms'] = [int(x) for x in _temp]
        params['id'] = env_name 

        params['log_dir'] = os.path.join(params['log_dir'], env_name)
        ...
        cmd = "python {RUN_FILE} \
                ...
                --im-lr {im_lr} \
                --pg-lr {pg_lr} \
                --beta-im-ep {beta_im_ep} \
                --beta-im-start {beta_im_start} \
                --beta-im-end {beta_im_end} \
                --beta-rl {beta_rl} \
                ...
                --weigh-class {weigh_class} \
                --gpu {gpu} \
                --log-dir {log_dir} \
                --reinforce-mode {reinforce_mode} \
                --reinforce-norm {reinforce_norm} \
                --val-frequency {val_frequency} \
                --n-policies {n_policies} \
                --max-steps {max_steps}".format(**params)
